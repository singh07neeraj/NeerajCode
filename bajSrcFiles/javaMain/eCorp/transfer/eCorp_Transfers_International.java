 ...
 * @author baj80000892
 */
public class eCorp_Transfers_International extends TestBase {

	public static Boolean eCorp_Transfers_International_Functions(String TCName, int ScenarioCount, Object[] tCsDataset) throws Exception {
		...
		try {
			...
			OpenEcorpMenues.openTransfersMenu("International");

			if (ZeroBalAcc.equalsIgnoreCase("true")) {

				Log.info("Selecting zero balance account number..Please make sure this account has Zero balance. " + Input.ReadTestData(AppData.accountSet, "ZeroBalAccNumber"));
				...
				Log.pass("Selected Account Number is :" + Input.ReadTestData(AppData.accountSet, "ZeroBalAccNumber"));
			}
			...
			// User trying to complete transaction without checking terms and conditions.
				...
				if (AmountLessThan100USD.contentEquals("true")) {
					...
					Utils.sendKeys(By.xpath(getObj("Propval1", "AmountTxt", "JOL_INTLTransfer")), "0.50", " Transfer Amount"); // Amount as 1 will be less than 100 USD in any currency hence can eb used to
																																// this scenario.
				} else {
					Utils.sendKeys(By.xpath(getObj("Propval1", "AmountTxt", "JOL_INTLTransfer")), AppData.getAmount_International(), " Transfer Amount");
				}
			...
			if (!TxfrAmountType.equalsIgnoreCase("SAR")) {
				...
				Log.pass("Selected currency transfer type is : "+TxfrAmountType);
			...
			if (Integer.parseInt(AddAnotherTxn) > 0) {
				...
				// table.
				eCorpCommonFunctions.addAnotherInternationalTxn(Integer.parseInt(AddAnotherTxn), FromAcc, TxfrAmountType, ExecutionTime, NextDate, FutureDate);
			...
			if (AmountLessThan100USD.contentEquals("true")) {
				...
				try {
					...
					Log.fail(" Internation transfer has worked successfully against expectations. Should have failed.Message displayed is : " + Utils.getText(By.xpath(getObj("Propval1", "SuccessMessage", "JOL_INTLTransfer"))));
					Utils.logFailImage("Transfer Result-Fail");
			...
			try {
				eleSize = Utils.getSize(By.xpath(getObj("Propval1", "SuccessMessage", "JOL_INTLTransfer")));
			} catch (Exception e) {
				gException = e;
			}
			...
			if (eleSize == 0) {
				...
				String Fail = Utils.getText(By.xpath(getObj("Propval1", "failMessage", "JOL_INTLTransfer")));
				Utils.logFailImage("Transfer Result");
				...
				String successMsg = Utils.getText(By.xpath(getObj("Propval1", "SuccessMessage", "JOL_INTLTransfer")));
				Log.pass(" Internation transfer has passed. Message displayed is : : " + successMsg);
				Utils.logPassImage("Transfer Result");

			

				// Code to perform additional actions like sendemail, print etc. Will work only
				// when the additional options are presen on screen. Example json below.
				// {"TransferType":"ForeignToBeneficiary","EXEC":"1","Procced":"1","Confirm":"1","NextDate":"10","FutureDate":"120","AfterTxfrAdditionalOptions":"true"}

				if (AdditionalActions.equalsIgnoreCase("true")) {
					
					Log.pass("Started saving transaction after selecting tag name and entering remrks");
					// Enter tag
			...
			if (Integer.parseInt(NewTxn) == 1) {
				Utils.scrollDownVertically();
				Utils.click(By.xpath(getObj("Propval1", "newTxn", "JOL_INTLTransfer")), "Add new Transaction button");
