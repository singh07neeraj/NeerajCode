...
public class FailedRequests extends TestBase {

	@SuppressWarnings("unused")
	public static Boolean FailedRequestsFunc(String TCName, int ScenarioCount, Object[] tCsDataset) throws Exception {
		...
		try {
			...
			OpenEcorpMenues.PendingActionsMenu("FailedRequests");
			...
			try {
				...
				Log.pass("No pending authorization requests found on screen.. skipping remaining test steps/cases.");
				return true;
			}

			Utils.mouseHover(By.xpath(getObj("Propval1", "DetailsShow", "PendingActions")));
			Log.pass("On Mouse Hover, Pending action details are shown successfully");
			Utils.logPassImage("Mouse Hover- Failed Requests");
			Utils.wait(4);
			...
			try {
				...
				Log.fail("Pending Action/s approval failed . .error... " + Utils.getText(By.xpath(getObj("Propval1", "Message", "PendingActions"))));
			...
			if (Home.equalsIgnoreCase("true")) {
				Utils.click(By.xpath(getObj("Propval1", "HomeBtnF", "PendingActions")), "Home ");
			}
