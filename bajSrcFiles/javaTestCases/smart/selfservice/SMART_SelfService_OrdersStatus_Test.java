...
public class SMART_SelfService_OrdersStatus_Test extends TestBase{


	@Test
	public void OrdersStatus(){
		
		TCName = "Smart Self Service - Orders Status";
		extentSetUp();
		...
		if(isMasterClassRun) {
			...
			for (scenarioCount = 1; scenarioCount <= RowCount(TCName); scenarioCount++) {
			try {
				javaMain.smart.selfservice.SMART_SelfService_OrdersStatus_Func.OrdersStatus(TCName,scenarioCount,dataset);
