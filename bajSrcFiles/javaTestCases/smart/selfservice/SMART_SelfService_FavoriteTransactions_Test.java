...
public class SMART_SelfService_FavoriteTransactions_Test extends TestBase{


	@Test
	public void FavoriteTransactions(){
		
		TCName = "Smart Self Service - FavoriteTransactions";
		extentSetUp();
		...
		if(isMasterClassRun) {
			...
			for (scenarioCount = 1; scenarioCount <= RowCount(TCName); scenarioCount++) {
			try {
				javaMain.smart.selfservice.SMART_SelfService_CommonFunction_Func.FavoriteTransactions(TCName,scenarioCount,dataset);
