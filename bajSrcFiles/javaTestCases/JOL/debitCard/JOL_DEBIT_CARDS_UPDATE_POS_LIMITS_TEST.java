...
import javaMain.JOL.debitCard.JOL_DEBIT_CARDS_UPDATE_POS_LIMITS_FUNC;

public class JOL_DEBIT_CARDS_UPDATE_POS_LIMITS_TEST extends TestBase

{
	...
	public void JOL_DEBIT_CARDS_DETAIL_INQUIRY() {
		...
		TCName = "JOL Debit Cards-update POS Limits";
		extentSetUp();
		...
		if (isMasterClassRun) {
			...
			for (scenarioCount = 5; scenarioCount <=RowCount(TCName); scenarioCount++) {
				...
				try {
					...
					JOL_DEBIT_CARDS_UPDATE_POS_LIMITS_FUNC.updatePOSLIMITS(TCName, scenarioCount, dataset);
